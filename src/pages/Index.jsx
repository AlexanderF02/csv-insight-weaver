...
const Index = () => {
  ...
  const handleDataLoaded = (csvData) => {
    ...
    if (csvData && csvData.headers && csvData.rows && csvData.rows.length > 0) {
      ...
      const pieChartData = Object.entries(barChartData).map(
        ([company, amount]) => ({
          name: company,
          value: amount,
        })
      ...
      const lineChartFormattedData = Object.entries(lineChartData).map(
        ([date, amount]) => ({
          date,
          value: amount,
        })
      ...
      // Set chart data
      setChartData({
        bar: Object.entries(barChartData).map(([name, value]) => ({
          name,
          value,
        })),
        pie: pieChartData,
      ...
      const uniqueCompanies = new Set(
        csvData.rows.map((row) => row[companyColumn])
      ).size;
  ...
  return (
    <div className="min-h-screen flex flex-col bg-appGray-light dark:bg-gray-900 dark:text-white">
      ...
      <div className="flex items-center mt-[40px] mb-[40px] space-x-2 ml-4">
