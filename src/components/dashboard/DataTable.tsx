...
const DataTable = ({ data }: DataTableProps) => {
  ...
  return (
    <div className="bg-dark-400 rounded-lg p-6">
      <div className="mb-6">
        <h2 className="text-xl font-semibold mb-4 text-white">Invoice Data</h2>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-dark-500 text-gray-400">
            <tr>
              {headers.map((header) => (
          ...
          <tbody className="divide-y divide-gray-800">
            {filteredData.map((row, rowIndex) => (
              <tr key={rowIndex} className="hover:bg-dark-300">
                {headers.map((header) => (
                  <td key={`${rowIndex}-${header}`} className="px-4 py-3 text-gray-300">
