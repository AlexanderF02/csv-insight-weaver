...
const DataTable = ({ data }) => {
    ...
    const processedData = useMemo(() => {
        ...
        if (sortField) {
            filtered = [...filtered].sort((a, b) => {
                ...
                if (!isNaN(parseFloat(a[sortField])) && !isNaN(parseFloat(b[sortField]))) {
    ...
    if (!data || !data.headers || !data.rows) {
        return <p className="text-center p-4">No data available. Please upload a CSV file.</p>;
    }
                    ...
                    <Table>
                        ...
                        <TableBody>
                            {paginatedData.length > 0 ? (
                                ...
                                <TableRow>
                                    <TableCell colSpan={data.headers.length} className="text-center">
                                        No results found
                                    </TableCell>
                                </TableRow>
                            )}
                ...
                {totalPages > 1 && (
                    <div className="flex items-center justify-between mt-4">
                        <p className="text-sm text-appGray-dark">
                            Showing {startIndex + 1} to {Math.min(startIndex + rowsPerPage, processedData.length)} of {processedData.length} entries
                        </p>
